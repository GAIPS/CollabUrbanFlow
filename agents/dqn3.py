...
class DQN3Lightning(pl.LightningModule):
    ...
    * ReplayBuffer: for storing experiences.
    ...
    def __init__(self, env, device, replay_size=200, warm_start_steps=0,
        ...
        super(DQN3Lightning, self).__init__(**kwargs)
    ...
    def reset(self, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        if self.num_episodes > 0:
            state_dict = torch.load(self.save_path / str(self.timestep) / f'DQN3.chkpt')
    ...
    def loss_step(self, batch):
        ...
        * loss: torch.tensor([B])
        ...
        for n_a in range(self.n_agents):
            ...
            state_action_values.append(q_values.gather(-1, u).squeeze(-1))
            with torch.no_grad():
                next_state_values, _ = self.target_net(y, n_a).max(-1)
                next_state_values = next_state_values.detach()


            expected_state_action_values.append(next_state_values * self.gamma + v)

        import ipdb; ipdb.set_trace()
        predictions = torch.cat(state_action_values)
        targets =  torch.cat(expected_state_action_values)
    ...
    def save_checkpoint(self, chkpt_dir_path, chkpt_num):
        file_path = Path(chkpt_dir_path) / str(chkpt_num) / f'DQN3.chkpt'
...
def load_checkpoint(env, chkpt_dir_path, rollout_time=None, network=None, chkpt_num=None):
    ...
    state_dict = torch.load(chkpt_path / f'DQN3.chkpt')
